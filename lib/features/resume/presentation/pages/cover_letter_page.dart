...
class _CoverLetterPageState extends State<CoverLetterPage>
  ...
  Future<void> _generate() async {
    ...
    if (premium.isPremium) {
      canProceed = await premium.consumeAiCall();
      if (!canProceed && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('premium.ai_limit_reached'.tr())),
        );
        return;
      }
    } else {
      canProceed = await premium.consumeFreeTrial(UsageLimitService.featureCoverLetter);
      if (!canProceed) {
        if (!mounted) return;
...
''';
      ...
      await SharePlus.instance.share(ShareParams(
        ...
        text: 'Check out my cover letter generated by AI Resume Pro',
      ));
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
                ...
                return Transform.scale(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      boxShadow: [
                        BoxShadow(
                          color: c.primaryStart.withValues(alpha: 0.3 + _pulseController.value * 0.2),
                          blurRadius: 20,
                          spreadRadius: 2,
                        ),
                      ],

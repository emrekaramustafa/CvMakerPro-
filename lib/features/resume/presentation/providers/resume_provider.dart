...
class ResumeProvider extends ChangeNotifier {
  ...
  void addSkill(String skill) {
    ...
    // Add to skills
    final updatedList = List<String>.from(_currentResume!.skills);
    if (!updatedList.contains(skill)) {
      updatedList.add(skill);
      _updateResume(skills: updatedList);
      
      // Remove from missingKeywords if present
      if (_aiAnalysis != null && _aiAnalysis!['missingKeywords'] is List) {
        final missingList = List<String>.from(_aiAnalysis!['missingKeywords']);
        if (missingList.contains(skill)) {
          missingList.remove(skill);
          _aiAnalysis!['missingKeywords'] = missingList;
          notifyListeners();
        }
      }
    }
  ...
  // Sharing & PDF
  Future<File> generatePdfToFile(ResumeModel resume) async {
    final pdfService = PdfGeneratorService();
    return await pdfService.generateResumePdf(resume);
  }

  Future<void> shareResume(ResumeModel resume) async {
    _isLoading = true;
    notifyListeners();
    try {
      final file = await generatePdfToFile(resume);
      await Share.shareXFiles([XFile(file.path)], text: 'Check out my resume generated by AI Resume Pro');
    } catch (e) {
